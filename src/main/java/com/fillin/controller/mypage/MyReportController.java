...
public class MyReportController {
    ...
    @GetMapping("/count")
    public Response<ReportCountResponseDto> getReportCount(@AuthUser Long memberId) {
        return myReportService.countReport(memberId);
    }

    @Operation(summary = "카테고리별 제보 갯수 확인", description = "유저의 카테고리별 제보 갯수를 확인합니다.")
    @GetMapping("/category")
    public Response<MyReportCategoryResponseDto> getMyReportCategory(@AuthUser Long memberId) {
        return myReportService.getMyReportCategory(memberId);
    }

    @Operation(summary = "만료 예정 제보(간단)", description = "5일내로 만료 예정인 제보들")
    @GetMapping("/soon")
    public Response<ReportExpireSoonDto> getReportExpireSoon(@AuthUser Long memberId) {
    ...
    public Response<List<ReportExpireSoonDetailDto>> getReportExpireSoonDetail(@AuthUser Long memberId) {
    ...
    @GetMapping
    public Response<List<MyReportListResponseDto>> getMyReportList(@AuthUser Long memberId) {
        return myReportService.getMyReportList(memberId);
    }

    @Operation(summary = "나의 제보 (만료) ", description = "유저의 제보 중 만료된 제보를 조회합니다.")
    @GetMapping("/expired")
    public Response<List<MyReportListResponseDto>> getMyReportExpired(@AuthUser Long memberId) {
    ...
    public Response<String> deleteReport(@AuthUser Long memberId,@PathVariable Long reportId) {
        return myReportService.deleteMyReport(memberId,reportId);
    }

    @Operation(summary = "저장한 제보", description = "유저가 저장한 제보를 조회합니다.")
    @GetMapping("/like")
    public Response<List<MyReportListResponseDto>> getMyReportLike(@AuthUser Long memberId) {
